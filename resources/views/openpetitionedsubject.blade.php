    ...
    <div id="page-wrapper">
        <div class="container-fluid">
            ...
            @foreach($subj as $subj)
                <?php
                $course = $subj->destitle;
                $courses = $subj->coursenumber;
                $t = $subj->term;
                ?>
                <div class="white-box" style="width: 85%; margin: 0 auto; height: 350px;">
                    <div style=" margin:0 auto; width:85%;">
                        ...
                        <div style="position: absolute; margin-left: 2%; font-size: 120%;">
                            <p>Course Number: {{$subj->coursenumber}}</p>
                            <p>Descriptive Title: {{$subj->destitle}}</p>

                        </div>
                        <p style="position: absolute; margin-left: 45%; font-size: 120%;">Units: {{$subj->units}}</p>
                        <p style="position: absolute; margin-left: 45%; font-size: 120%; margin-top: 2%;">Term: {{$subj->term}}</p>
                        <?php
                        ...
                        @foreach($subject as $subject)
                            <?php
                            $subj = $subject->subjectID;
                            ?>
                        @endforeach
                        <form method="post" action="/petitions/{{$subj}}/{{$t}}">
                            <select class="form-control row b-none" name="cour" id="cour" style="width: 20%; position: relative; margin-top:7%; margin-left: 2%; font-size: 120%; ">
                                <?php
